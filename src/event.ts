...
import { setTimeoutAsync, removeValue } from './utils.js';
...
export class Event<T = unknown, R = unknown> extends Callable<[T], Promise<(void | Awaited<R>)[]>> implements AsyncIterable<T>, PromiseLike<T> {
   ...
   */
  private listeners: Ring<Listener<T, R>>;
  ...
  constructor(dispose?: Callback) {
    const listeners = new Ring<Listener<T, R>>();
    // passes listeners exceptions to catch method
    super((value: T): Promise<(void | Awaited<R>)[]> => Promise.all(listeners.toArray().map(async (listener) => listener(await value))));
